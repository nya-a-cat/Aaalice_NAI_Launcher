...
class GalleryVibeFamilyRepository {
  ...
  Future<void> initialize() =>
      gateway.execute('createVibeFamilyTables', (db) async {
        await db.execute(
          'CREATE TABLE IF NOT EXISTS gallery_vibe_families '
          '(id TEXT PRIMARY KEY, name TEXT NOT NULL)',
        );
        await db.execute(
          'CREATE TABLE IF NOT EXISTS gallery_vibe_family_members '
          '(vibe_hash TEXT PRIMARY KEY, family_id TEXT NOT NULL)',
        );
        await db.execute(
          'CREATE TABLE IF NOT EXISTS gallery_vibe_separations '
          '(pair_key TEXT PRIMARY KEY)',
        );
        await db.execute(
          'CREATE TABLE IF NOT EXISTS gallery_vibe_style_features '
      ...
      families: families
          .map(
            (f) => VibeFamily(
              ...
              members: members
                  .where((m) => m['family_id'] == f['id'])
                  .map((m) => m['vibe_hash']! as String)
                  .toSet(),
            ),
          )
          .toList(),
  ...
  Future<void> merge(
    ...
    for (var i = 0; i < ordered.length; i++) {
      for (var j = i + 1; j < ordered.length; j++) {
        if (state.separations.contains(
          VibeFamilyState.pairKey(ordered[i], ordered[j]),
        )) {
          throw StateError('vibe_family_separation_conflict');
        }
      }
    }
    final id = oldIds.isEmpty
        ? const Uuid().v4()
        : (oldIds.toList()..sort()).first;
    for (final oldId in oldIds) {
      await txn.delete(
        'gallery_vibe_family_members',
        where: 'family_id = ?',
        whereArgs: [oldId],
      );
      await txn.delete(
    ...
    for (final hash in members) {
      batch.insert('gallery_vibe_family_members', {
        'vibe_hash': hash,
        'family_id': id,
      });
    }
  ...
  Future<void> restorePair(String pair) =>
      gateway.execute('restoreVibePair', (db) async {
        await db.delete(
          'gallery_vibe_separations',
          where: 'pair_key = ?',
          whereArgs: [pair],
  ...
  Future<Map<String, List<List<double>>>> cachedFeatures(
    int version,
    List<int> imageIds,
    ...
    for (var offset = 0; offset < imageIds.length; offset += 256) {
      ...
      final rows = await db.query(
        'gallery_vibe_style_features',
        where:
            'version = ? AND image_id IN (${List.filled(ids.length, '?').join(',')})',
        whereArgs: [version, ...ids],
      );
      for (final r in rows) {
        try {
          result[r['cache_key']!
              as String] = (jsonDecode(r['features']! as String) as List)
              .map(
                (g) => (g as List).map((v) => (v as num).toDouble()).toList(),
              )
              .toList();
        } catch (_) {
  ...
  Future<List<Map<String, Object?>>> corpusPage(
    int afterId, {
    int limit = 128,
  }) => gateway.execute(
    'queryVibeStyleCorpus',
    (db) => db.rawQuery(
      r'''
