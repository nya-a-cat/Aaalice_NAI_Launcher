...
class QuickTagCloudRelayPanel extends ConsumerWidget {
  ...
  Future<void> _planAction(
    ...
    if (action == 'delete') {
      final confirmed = await ThemedConfirmDialog.showDelete(
        context: context,
        itemName: doc.activePlan.name.isEmpty
            ? l.common_default
            : doc.activePlan.name,
      );
      if (confirmed && context.mounted) {
        await _run(context, controller.deletePlan(doc.activePlanId));
      }
      return;
    }
    final name = await _askPlanName(
      context,
      title: action == 'new' ? l.qtcRelay_newPlan : l.qtcRelay_renamePlan,
    ...
    await _run(
      context,
      action == 'new'
          ? controller.createPlan(name)
          : controller.renamePlan(doc.activePlanId, name),
    );
  }
  ...
  Future<void> _deleteBlock(
    ...
    if (confirmed && context.mounted) {
      await _run(
        context,
        ref
            .read(quickTagCloudRelayProvider.notifier)
            .removeBlock(planId, block.id),
      );
    }
  }
      ...
      children: [
        ...
        PopupMenuButton<String>(
          ...
          itemBuilder: (context) => [
            ...
            PopupMenuItem(
              value: 'delete',
              enabled: doc.plans.length > 1,
              child: Text(l.qtcRelay_deletePlan),
            ),
          ],
        ),
      ],
  ...
  Widget _recovery(BuildContext context, WidgetRef ref, bool busy) => Column(
        ...
        onPressed: busy
            ...
            : () async {
                final confirmed = await ThemedConfirmDialog.show(
                  context: context,
                  title: context.l10n.qtcRelay_recover,
      ...
      children: [
        ...
        if (state.saveFailed)
          Padding(
            ...
            child: Text(
              l.qtcRelay_saveFailed,
              style: TextStyle(color: Theme.of(context).colorScheme.error),
            ),
          ),
        if (state.isCorrupt)
          _recovery(context, ref, state.isSaving)
        else ...[
          ...
          Align(
            ...
            child: TextButton.icon(
              onPressed: state.isSaving
                  ? null
                  : () => showRelayBlockEditor(context, planId: plan.id),
              icon: const Icon(Icons.add),
              label: Text(l.qtcRelay_addBlock),
            ),
          ),
          if (plan.blocks.isEmpty)
            Padding(
              padding: const EdgeInsets.symmetric(vertical: 24),
              child: Text(l.qtcRelay_empty),
            ),
          for (var index = 0; index < plan.blocks.length; index++)
            Padding(
              ...
              child: RelayBlockTile(
                ...
                locked: !plan.blocks[index].allowedBy(ratings),
                busy: state.isSaving,
                canMoveUp: index > 0,
                canMoveDown: index + 1 < plan.blocks.length,
                onToggle: () => _run(
                  context,
                  controller.toggleBlock(plan.id, plan.blocks[index].id),
                ),
                onEdit: () => showRelayBlockEditor(
                  context,
                  planId: plan.id,
                  block: plan.blocks[index],
                ),
                onDelete: () => _deleteBlock(
                  ...
                  plan.id,
                  plan.blocks[index],
                  !plan.blocks[index].allowedBy(ratings),
                ),
                onMoveUp: () => _run(
                  context,
                  controller.moveBlock(plan.id, plan.blocks[index].id, -1),
                ),
                onMoveDown: () => _run(
                  context,
                  controller.moveBlock(plan.id, plan.blocks[index].id, 1),
