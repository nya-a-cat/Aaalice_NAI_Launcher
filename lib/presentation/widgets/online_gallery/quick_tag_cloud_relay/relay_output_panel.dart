...
class RelayOutputPanel extends ConsumerWidget {
  ...
  QuickTagCloudRelayOutput _output(WidgetRef ref, {bool forGeneration = false}) {
    final state = ref.read(quickTagCloudRelayProvider);
    if (state.isCorrupt || state.isSaving) throw StateError('Relay unavailable');
  ...
  Future<void> _copy(BuildContext context, WidgetRef ref, int channel) async {
    try {
      ...
      final text = switch (channel) {
        0 => output.positive, 1 => output.negative, _ => output.allText,
      };
      if (text.isEmpty) return;
      await Clipboard.setData(ClipboardData(text: text));
      if (context.mounted) AppToast.success(context, context.l10n.common_copied);
    } catch (_) {
  ...
  Future<void> _send(BuildContext context, WidgetRef ref) async {
    final confirmed = await ThemedConfirmDialog.show(
      context: context, title: context.l10n.qtcRelay_send,
    ...
    try {
      ...
      final existing = ref.read(characterPromptNotifierProvider).characters.length;
      ...
      for (final character in output.characters) {
        characters.addCharacter(
          CharacterGender.other, name: character.label,
          prompt: character.positive, negativePrompt: character.negative,
        );
      }
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    try {
      output = const QuickTagCloudRelayService().compile(
        doc.activePlan, format: doc.format, join: doc.join, allowedRatings: ratings,
      );
    } catch (_) {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      if (doc.format == QuickTagCloudRelayFormat.plain) Text(l.qtcRelay_plainHint),
  ...
  Widget _settings(BuildContext context, WidgetRef ref,
      QuickTagCloudRelayDocument doc, bool busy) {
    ...
    return Wrap(spacing: 16, runSpacing: 8, children: [
        DropdownButton<QuickTagCloudRelayFormat>(
          value: doc.format,
          items: [
            const DropdownMenuItem(value: QuickTagCloudRelayFormat.nai, child: Text('NAI')),
            const DropdownMenuItem(value: QuickTagCloudRelayFormat.sd, child: Text('SD')),
            DropdownMenuItem(value: QuickTagCloudRelayFormat.plain, child: Text(l.qtcRelay_plain)),
          ],
          onChanged: busy ? null : (value) {
            if (value != null) _setting(context, controller.setFormat(value));
          },
        ),
        DropdownButton<QuickTagCloudRelayJoin>(
          value: doc.join,
          items: [
            DropdownMenuItem(value: QuickTagCloudRelayJoin.comma, child: Text(l.qtcRelay_comma)),
            DropdownMenuItem(value: QuickTagCloudRelayJoin.newline, child: Text(l.qtcRelay_newline)),
          ],
          onChanged: busy ? null : (value) {
            if (value != null) _setting(context, controller.setJoin(value));
          },
        ),
    ]);
  }
  ...
  Widget _preview(BuildContext context, QuickTagCloudRelayOutput? output) {
    final l = context.l10n;
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      if (output == null) Text(l.common_error,
        style: TextStyle(color: Theme.of(context).colorScheme.error)),
      if ((output?.lockedCount ?? 0) > 0) Text('${l.qtcRelay_locked}: ${output!.lockedCount}'),
      if ((output?.mergedCount ?? 0) > 0) Text('${l.qtcRelay_merged}: ${output!.mergedCount}'),
      if (output?.characters.isNotEmpty ?? false) Text(l.qtcRelay_charactersNote),
      if (output != null && !output.isEmpty)
        ExpansionTile(
          title: Text(l.prompt_finalPrompt),
          children: [Padding(
            padding: const EdgeInsets.only(bottom: 16),
            child: Align(alignment: Alignment.centerLeft, child: SelectableText(output.allText)),
          )],
        ),
    ]);
  }
  ...
  Widget _actions(BuildContext context, WidgetRef ref,
      QuickTagCloudRelayOutput? output, bool busy) {
    final l = context.l10n;
    return Wrap(spacing: 8, runSpacing: 8, children: [
        TextButton.icon(
          onPressed: busy || (output?.positive.isEmpty ?? true) ? null : () => _copy(context, ref, 0),
          icon: const Icon(Icons.copy_outlined), label: Text(l.qtcRelay_copyPositive),
        ),
        TextButton.icon(
          onPressed: busy || (output?.negative.isEmpty ?? true) ? null : () => _copy(context, ref, 1),
          icon: const Icon(Icons.copy_outlined), label: Text(l.qtcRelay_copyNegative),
        ),
        FilledButton.tonalIcon(
          onPressed: busy || (output?.isEmpty ?? true) ? null : () => _copy(context, ref, 2),
          icon: const Icon(Icons.copy_all_outlined), label: Text(l.qtcRelay_copyAll),
        ),
        FilledButton.icon(
          onPressed: busy || (output?.isEmpty ?? true) ? null : () => _send(context, ref),
          icon: const Icon(Icons.north_east), label: Text(l.qtcRelay_send),
        ),
    ]);
  }
}
