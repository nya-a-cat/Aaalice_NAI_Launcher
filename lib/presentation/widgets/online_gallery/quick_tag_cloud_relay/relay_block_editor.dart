...
class _RelayBlockEditorState extends ConsumerState<_RelayBlockEditor> {
  ...
  late final _positive = TextEditingController(text: widget.block?.positive ?? '');
  late final _negative = TextEditingController(text: widget.block?.negative ?? '');
  late final _weight = TextEditingController(text: '${widget.block?.weight ?? 1}');
  bool _saving = false;
  String? _error;

  @override
  void dispose() {
    _title.dispose(); _positive.dispose(); _negative.dispose(); _weight.dispose();
    super.dispose();
  }
  ...
  Future<void> _save() async {
    ...
    if (original != null && !original.allowedBy(ref.read(quickTagCloudRelayAllowedRatingsProvider))) {
      setState(() => _error = context.l10n.qtcRelay_locked);
      return;
    }
    setState(() { _saving = true; _error = null; });
    final block = (original ?? QuickTagCloudRelayBlock(
      id: const Uuid().v4(), title: '',
    )).copyWith(
      title: _title.text.trim(), positive: _positive.text,
      negative: _negative.text, weight: weight,
    );
    try {
  ...
  Widget _field(String label, TextEditingController controller, {bool multiline = false}) =>
      Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          ThemedInput(
            controller: controller, enabled: !_saving,
  ...
  Widget build(BuildContext context) {
    ...
    final characters = widget.block?.characters ?? const <QuickTagCloudRelayCharacter>[];
      ...
      child: AlertDialog(
        ...
        content: SizedBox(
          ...
          child: SingleChildScrollView(child: Column(
            ...
            children: locked ? [Text(l.qtcRelay_locked)] : [
              _field(l.qtcRelay_blockTitle, _title),
              _field(l.prompt_positivePrompt, _positive, multiline: true),
              _field(l.prompt_negativePrompt, _negative, multiline: true),
              Text(l.qtcRelay_weight, style: Theme.of(context).textTheme.labelLarge),
              const SizedBox(height: 8),
              ThemedInput(
                controller: _weight, enabled: !_saving,
              ...
              if (characters.isNotEmpty) ...[
                ...
                for (final character in characters)
                  ExpansionTile(
                    title: Text(character.label.isEmpty ? l.prompt_characterPrompts : character.label),
                    children: [
                      ListTile(title: Text(l.prompt_positive), subtitle: SelectableText(character.positive)),
                      ListTile(title: Text(l.prompt_negative), subtitle: SelectableText(character.negative)),
                    ],
                  ),
              ],
              if (_error != null) ...[
        ...
        actions: [
          TextButton(onPressed: _saving ? null : () => Navigator.pop(context), child: Text(l.common_cancel)),
          FilledButton(onPressed: _saving || locked ? null : _save,
