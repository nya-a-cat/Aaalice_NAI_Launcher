...
final vibeFamilyProvider = ChangeNotifierProvider.autoDispose<VibeFamilyController>((ref) {
  final gallery = GalleryDataSource();
  return VibeFamilyController(gallery.vibeFamilies,
    initializeGallery: () => ref.read(localGalleryVibeProvider.notifier).initialize());
});
...
class VibeFamilyController extends ChangeNotifier {
  ...
  List<VibeStyleMatch> get suggestions => result?.matches.where((m) =>
    !decisions.excludes(m.left, m.right)).toList() ?? [];
  ...
  Future<void> retry() { _initialization = null; loading = true; _notify(); return initialize(); }

  Future<void> analyze() async {
    if (analyzing || saving || loading) return;
    analyzing = true; cancelled = false; error = null; progress = null; _notify();
    try {
      final next = await _analysis.analyze(onProgress: (value) {
        progress = value; _notify();
      });
      if (_alive) result = next;
    } on VibeStyleCancelled { cancelled = true; }
    catch (_) { error = 'analysis'; }
    finally { analyzing = false; progress = null; _notify(); }
  }

  void cancel() => _analysis.cancel();
  Future<void> merge(Set<String> hashes, String name) =>
    _save(() => repository.merge(hashes, name));
  Future<void> separate(String a, String b) => _save(() => repository.separate(a, b));
  Future<void> split(String hash) => _save(() => repository.split(hash));
  Future<void> rename(String id, String name) => _save(() => repository.rename(id, name));
  Future<void> restore(String pair) => _save(() => repository.restorePair(pair));
  Future<void> _save(Future<void> Function() operation) async {
    if (saving) return;
    saving = true; error = null; _notify();
    try { await operation(); await _reload(); }
    catch (e) { error = e.toString().contains('separation_conflict') ? 'conflict' : 'save'; }
    finally { saving = false; _notify(); }
  }
  void _notify() { if (_alive) notifyListeners(); }
  @override
  void dispose() { _alive = false; _analysis.cancel(); super.dispose(); }
}
