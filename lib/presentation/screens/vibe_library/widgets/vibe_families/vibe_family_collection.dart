...
class VibeFamilyCollection extends StatelessWidget {
  const VibeFamilyCollection({super.key, required this.controller, required this.onOpen,
    required this.onRename, required this.separated});
  ...
  Widget build(BuildContext context) {
    ...
    if (separated) {
      ...
      return ListView.builder(itemCount: pairs.length, itemBuilder: (context, i) {
        ...
        return ListTile(
          ...
          trailing: IconButton(tooltip: l.vibeFamily_restore,
            onPressed: controller.saving ? null : () => controller.restore(pairs[i]),
            icon: const Icon(Icons.undo)),
        );
      });
    }
    if (state.families.isEmpty) return Center(child: Padding(
      padding: const EdgeInsets.all(24), child: Text(l.vibeFamily_noFamilies)));
    return ListView.builder(itemCount: state.families.length,
      itemBuilder: (context, index) {
        final family = state.families[index];
        return ExpansionTile(
          key: PageStorageKey(family.id), initiallyExpanded: true,
          title: Text(family.name), subtitle: Text(l.vibeFamily_memberCount(family.members.length)),
          trailing: IconButton(tooltip: l.vibeFamily_rename,
          ...
          children: [
            LayoutBuilder(builder: (context, constraints) {
              ...
              return Wrap(children: members.map((hash) {
                ...
                return SizedBox(width: width, child: Padding(
                  padding: const EdgeInsets.all(8), child: Column(children: [
                    VibeFamilyPreview(path: summary?.previewPath,
                      label: 'Vibe ${summary?.shortHash ?? (hash.length > 10 ? hash.substring(0,10) : hash)}',
                      onTap: () => onOpen(hash)),
                    TextButton(onPressed: controller.saving ? null : () => controller.split(hash),
