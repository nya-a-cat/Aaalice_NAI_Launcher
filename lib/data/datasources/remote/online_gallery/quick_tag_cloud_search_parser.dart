...
class QuickTagCloudSearchParser {
  ...
  static QuickTagCloudSearchPlan parse(
    ...
    if (raw.trim().isNotEmpty &&
        ...
        issues.isEmpty) {
      issues.add(
        const QuickTagCloudSearchIssue('empty_search', '请输入至少一个有效关键词或筛选条件'),
      );
    }
  ...
  static void _consumeStandaloneFilter(
    ...
    if (remainder.startsWith('"') || remainder.startsWith("'")) {
      ...
      if (tokens.length == 1) {
        ...
        issues.add(
          QuickTagCloudSearchIssue('invalid_filter', '筛选条件应为单个“字段:值”条件', value),
        );
      }
      return;
    }
  ...
  static void _addTerms(
    ...
    if (token.quoted && token.value.trim().isEmpty) {
      issues.add(
        const QuickTagCloudSearchIssue('empty_text_condition', '搜索短语不能为空'),
      );
    }
    ...
    terms.addAll(
      values.map(normalizeQuickTagCloudSearchText).where((v) => v.isNotEmpty),
    );
  }
  ...
  static void _addFieldFilter(
    ...
    if (field == 'directory') {
      ...
      if (split < 1 || value.substring(split + 1).trim().isEmpty) {
        issues.add(
          QuickTagCloudSearchIssue('empty_filter', '精确目录筛选缺少法典或目录短码', value),
        );
        return;
      }
      _append(
        QuickTagCloudSearchFilter(
          field: field,
          value: normalizeQuickTagCloudSearchText(value.substring(split + 1)),
    ...
    if (field == 'has' || field == 'fav') {
      ...
      if (boolean == null) {
        issues.add(
          QuickTagCloudSearchIssue(
            'invalid_enum',
            field == 'has' ? '图片筛选只能是“有图”或“无图”' : '收藏筛选只能是“已收藏”或“未收藏”',
            value,
          ),
        );
        return;
      ...
      if (type == null) {
        issues.add(
          QuickTagCloudSearchIssue('invalid_enum', '类型只能是法典、画风、构图或图包', value),
        );
        return;
      }
      value = type;
  ...
  static void _append(
    QuickTagCloudSearchFilter filter,
    List<QuickTagCloudSearchFilter> filters,
    List<QuickTagCloudSearchIssue> issues,
  ) {
  ...
  static List<_Token> _scan(String raw, List<QuickTagCloudSearchIssue> issues) {
    ...
    if (quote.isNotEmpty) {
      issues.add(
        QuickTagCloudSearchIssue('unclosed_quote', '引号没有闭合，请补全后再搜索', raw),
      );
    }
    return tokens;
  }
