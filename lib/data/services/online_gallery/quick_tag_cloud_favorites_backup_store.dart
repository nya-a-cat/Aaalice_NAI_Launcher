...
typedef QuickTagCloudBackupEntryResolver =
    Future<GalleryDetail?> Function(
      String codexId,
      String entryId,
      CancelToken token,
    );

typedef QuickTagCloudBackupCommunityResolver =
    Future<GalleryDetail?> Function(String entryId, CancelToken token);

class QuickTagCloudBackupPreview {
  QuickTagCloudBackupPreview({
    required this.previousRaw,
...
class QuickTagCloudFavoritesBackupStore {
  ...
  List<OnlineGalleryFavoriteRecord> _records() => repository
      .query(
  ...
  Future<QuickTagCloudBackupPreview> prepare(
    ...
    for (final item in QuickTagCloudBackupPlan.atlas(incoming)) {
      ...
      resolved[key] = OnlineGalleryFavoriteRecord.fromDetail(
        detail,
    ...
    return QuickTagCloudBackupPreview(
      previousRaw: current.previousRaw,
  ...
  Future<QuickTagCloudBackupPreview> _snapshot() async {
    ...
    final items = <String, Map<String, dynamic>>{
      for (final item in QuickTagCloudBackupPlan.atlas(document))
        if (!oldMappings.containsKey(
              QuickTagCloudFavoritesBackupCodec.keyOf(item),
            ) ||
            liveKeys.contains(
              oldMappings[QuickTagCloudFavoritesBackupCodec.keyOf(item)],
            ))
    ...
    final community = <String>{
      for (final id in QuickTagCloudBackupPlan.community(document))
        if (!oldMappings.containsKey(
              QuickTagCloudBackupPlan.communityKey(id),
            ) ||
            liveKeys.contains(
              oldMappings[QuickTagCloudBackupPlan.communityKey(id)],
            ))
          id,
    };
    final itemKeys = {
      ...items.keys,
      ...community.map(QuickTagCloudBackupPlan.communityKey),
    };
    ...
    if (document['memberships'] is List) {
      document['memberships'] = (document['memberships'] as List)
          .where((m) => items.containsKey(m['itemKey']))
          .toList();
    }
  ...
  Future<void> commit(
    ...
    final next = jsonEncode({
      ...
      'mappings': preview.nextMappings(plan),
    });
    final replacementId = const Uuid().v4();
    await storage.setSetting(
      journalKey,
      jsonEncode({
        'previous': preview.previousRaw,
        'next': next,
