...
class VibeStyleTexture {
  static List<double> describe(Float64List pixels, int width, int height) {
    ...
    final low = List.generate(side * side, (i) {
      ...
      final y = math.min(height - 1, ((i ~/ side + 0.5) * height / side).floor());
      return pixels[y * width + x];
    });
    final cosines = List.generate(side, (u) => List.generate(side,
    ...
    for (var v = 0; v < side; v++) {
      for (var u = 0; u < side; u++) {
        ...
        final radial = u+v;
        final band = radial < 4 ? 0 : radial < 8 ? 1 : radial < 16 ? 2 : 3;
        bands[band] += coefficient * coefficient * (u == 0 || v == 0 ? 0.5 : 1);
      }
    }
    final total = bands.reduce((a,b) => a+b);
    if (total > 1e-12) {
      for (var i = 0; i < bands.length; i++) { bands[i] /= total; }
    } else {
      bands.fillRange(0,4,0);
    }
    return [contrast,homogeneity,energy,entropy,...bands].map((v) => v.clamp(0.0,1.0)).toList();
  }
}
