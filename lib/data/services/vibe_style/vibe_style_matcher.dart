...
class VibeStyleMatcher {
  static List<VibeStyleMatch> rank(
    List<VibeStyleSample> samples,
    ...
    final prototypes = {
      for (final code in codes)
        code: _medianFeatures(
          byCode[code]!.map((s) => features[s.cacheKey]!).toList(),
        ),
    };
    ...
    for (var i = 0; i < codes.length; i++) {
      for (var j = i + 1; j < codes.length; j++) {
        ...
        final groups = selected
            .map(
              (p) => _similarities(
                features[p.$1.cacheKey]!,
                features[p.$2.cacheKey]!,
              ),
            )
            .toList();
        ...
        if (groups.isEmpty &&
            byCode[a]!
                .map((s) => s.imageId)
                .toSet()
                .intersection(byCode[b]!.map((s) => s.imageId).toSet())
                .isNotEmpty) {
          continue;
        }
        final dims = groups.isEmpty
            ? _similarities(prototypes[a]!, prototypes[b]!)
        ...
        final stability = scores.length < 2
            ? 0.0
            : (1 - (scores.reduce(math.max) - scores.reduce(math.min))).clamp(
                0.0,
                1.0,
              );
        if (selected.isEmpty) {
        ...
        matches.add(
          VibeStyleMatch(
            ...
            dimensions: dims,
            recipeCount: controls.length,
            sameSeedCount: controls.isEmpty
                ? 0
                : selected
                      .where((p) => p.$1.seed != null && p.$1.seed == p.$2.seed)
                      .length,
            stability: stability,
            examples: selected.take(3).toList(),
          ),
        );
      }
    }
    ...
    return matches
        .where((m) {
          if ((used[m.left] ?? 0) >= 3 || (used[m.right] ?? 0) >= 3) {
            return false;
          }
        ...
        .toList()
      ..sort((a, b) {
        final controlled = (b.hasControls ? 1 : 0).compareTo(
          a.hasControls ? 1 : 0,
        );
        return controlled != 0
            ? controlled
  ...
  static List<double> _similarities(
    List<List<double>> a,
    List<List<double>> b,
  ) => List.generate(4, (g) {
    ...
    for (var i = 0; i < a[g].length; i++) {
      ...
      final delta =
          math.sqrt(math.max(0, a[g][i])) - math.sqrt(math.max(0, b[g][i]));
  ...
  static List<double> _weights(
    ...
    final adjusted = List.generate(
      4,
      (g) => base[g] / (0.1 + median(variability.map((v) => v[g]).toList())),
    );
    final sum = adjusted.reduce((a, b) => a + b);
    return adjusted.map((v) => v / sum).toList();
  }

  static List<List<double>> _medianFeatures(
    List<List<List<double>>> features,
  ) => List.generate(
    4,
    (g) => List.generate(
      features.first[g].length,
