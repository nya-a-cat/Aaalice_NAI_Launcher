...
class VibeStyleCorpus {
  static List<VibeStyleSample> parse(List<Map<String, Object?>> rows) {
    ...
    for (final row in rows) {
      try {
        ...
        final extra = row['extra_controls'] == null ? null :
        ...
        final required = ['model', 'sampler', 'steps', 'cfg_scale', 'width', 'height'];
        final known = required.every((k) => row[k] != null) && extra != null;
        final noReference = extra != null && [12,13,14,15,16]
            .every((i) => extra[i] == null || extra[i] == '' ||
                (extra[i] is List && (extra[i] as List).isEmpty));
        final complete = known && noReference && row['is_img2img'] != 1 &&
            row['raw_ref_count'] == refs.length && refs.every((r) =>
              r['strength'] is num && r['info'] is num && r['ordinal'] is num);
        final controls = [
          prompt, row['negative_prompt'],
          for (final key in required) row[key],
          row['noise_schedule'], row['cfg_rescale'], row['smea'], row['smea_dyn'], extra,
        ];
        for (final ref in refs) {
          // Order and values of the remaining Vibes must match for a controlled pair.
          final others = refs.where((r) => !identical(r, ref))
            .map((r) => [r['hash'], r['strength'], r['info'], r['ordinal']]).toList();
          final recipe = complete ? _hash([
            controls, ref['strength'], ref['info'], ref['ordinal'], others,
          ]) : '';
          samples.add(VibeStyleSample(
            imageId: (row['id']! as num).toInt(), path: row['file_path']! as String,
            size: (row['file_size']! as num).toInt(),
            modifiedAt: (row['modified_at']! as num).toInt(),
            hash: ref['hash'] as String, recipe: recipe, promptKey: promptKey,
  ...
  static List<VibeStyleSample> select(List<VibeStyleSample> all,
      {int maxImages = 1600, int perCode = 16}) {
    ...
    for (final s in all) { byCode.putIfAbsent(s.hash, () => []).add(s); }
    for (final bucket in byCode.values) {
      ...
      bucket..clear()..addAll(diverse.take(perCode));
    }
    final keys = byCode.keys.toList()..sort((a,b) {
    ...
    for (var round = 0; round < perCode; round++) {
      for (final code in activeKeys) {
        ...
        if (!imageIds.contains(s.imageId) && imageIds.length >= maxImages) continue;
