...
class TestGalleryGateway implements GalleryDatabaseGateway {
  ...
  final Database db;
  @override
  Future<T> execute<T>(String name,Future<T> Function(Database) operation,
    {Duration? timeout,int? maxRetries}) => operation(db);
  @override
  Future<T> executeTransaction<T>(String name,Future<T> Function(Transaction) operation,
    {Duration? timeout}) => db.transaction(operation);
}
...
void main() {
  ...
  tearDown(() => db.close());
  test('merge, rename and split persist and keep exact encoding records intact', () async {
    await db.execute('CREATE TABLE gallery_image_vibes (vibe_hash TEXT, vibe_encoding TEXT)');
    await db.insert('gallery_image_vibes',{'vibe_hash':'a','vibe_encoding':'original bytes'});
    ...
    var state = await GalleryVibeFamilyRepository(TestGalleryGateway(db)).load();
    ...
    expect(await db.query('gallery_image_vibes'),[{'vibe_hash':'a','vibe_encoding':'original bytes'}]);
  });
  test('separation blocks transitive family merge atomically until restored', () async {
  ...
  test('splitting two-member family removes empty family and remembers decision', () async {
    await repo.merge({'a','b'},'A');
    await repo.split('a');
  ...
  test('cache versions and malformed entries are isolated', () async {
    const sample = VibeStyleSample(imageId:1,path:'test',size:10,modifiedAt:100,
      hash:'a',recipe:'r',promptKey:'p',seed:1);
    await repo.saveFeatures(sample,1,[[0.5]]);
    expect((await repo.cachedFeatures(1,[1]))[sample.cacheKey],[[0.5]]);
    expect(await repo.cachedFeatures(2,[1]),isEmpty);
    expect(await repo.cachedFeatures(1,[2]),isEmpty);
    await db.update('gallery_vibe_style_features',{'features':'invalid json'});
    expect(await repo.cachedFeatures(1,[1]),isEmpty);
  });
  test('keyset corpus excludes large reference payloads and handles invalid JSON', () async {
    await db.execute('CREATE TABLE gallery_images (id INTEGER PRIMARY KEY, file_path TEXT, '
      'file_size INTEGER, modified_at INTEGER, created_at INTEGER, is_deleted INTEGER)');
    await db.execute('CREATE TABLE gallery_metadata (image_id INTEGER, prompt TEXT, '
    ...
    await db.execute('CREATE TABLE gallery_image_vibes (image_id INTEGER, vibe_hash TEXT, '
      'strength REAL, info_extracted REAL, ordinal INTEGER)');
    for (var id = 1; id <= 3; id++) {
      await db.insert('gallery_images',{'id':id,'file_path':'$id.png','file_size':10,
        'modified_at':100,'created_at':id,'is_deleted':0});
      await db.insert('gallery_metadata',{'image_id':id,'prompt':'test','model':'nai',
        'sampler':'euler','steps':28,'cfg_scale':5,'width':832,'height':1216,
        'is_img2img':0,'has_vibe':1,'raw_json':id == 3 ? 'broken' : jsonEncode({
          'reference_image_multiple':['encoding'],
          if (id == 2) 'director_reference_images':['x' * 100000],
        })});
      await db.insert('gallery_image_vibes',{'image_id':id,'vibe_hash':'a',
        'strength':1,'info_extracted':0.7,'ordinal':0});
    }
