...
Map<String, Object?> row(
  ...
  'is_img2img': 0,
  'seed': seed,
  'extra_controls': jsonEncode(List.filled(26, null)),
  'raw_ref_count': other == null ? 1 : 2,
  'refs': jsonEncode([
    {'hash': hash, 'strength': strength, 'info': 0.7, 'ordinal': 0},
    if (other != null)
      {'hash': other, 'strength': 1.0, 'info': 0.7, 'ordinal': 1},
  ]),
};

void main() {
  test(
    'controlled recipes include other Vibes, strength and full settings',
    () {
      final values = VibeStyleCorpus.parse([
        row(1, 'a', other: 'c'),
        row(2, 'b', other: 'c', seed: 2),
        row(3, 'b', other: 'd'),
      ...
      for (final s in values.skip(2)) {
        expect(s.recipe, isNot(values[0].recipe));
      }
    },
  );
  test(
    'missing, duplicate, img2img and character controls stay conservative',
    () {
      final rows = [
      ...
      expect(
        VibeStyleCorpus.parse(rows).map((s) => s.recipe),
        everyElement(isEmpty),
      );
      final extra = List<Object?>.filled(26, null)
        ..[0] = {
          'character_prompts': ['different'],
        };
      final variants = VibeStyleCorpus.parse([
        row(5, 'a'),
        {...row(6, 'b'), 'extra_controls': jsonEncode(extra)},
      ]);
      expect(variants[0].recipe, isNot(variants[1].recipe));
      expect(
        VibeStyleCorpus.parse([
          {'refs': 'broken'},
        ]),
        isEmpty,
      );
  ...
  test(
    'sampler selects diverse prompts within strict image and per-code budgets',
    () {
      final corpus = VibeStyleCorpus.parse(
        List.generate(
          800,
          (i) => row(i + 1, 'code-${i % 20}', prompt: 'prompt-${i ~/ 20}'),
        ),
      );
      final selected = VibeStyleCorpus.select(
        corpus,
        maxImages: 50,
        perCode: 4,
      );
      expect(
        selected.map((s) => s.imageId).toSet().length,
  ...
  test(
    'traditional descriptors are bounded, deterministic and input-preserving',
    () {
      final image = img.Image(width: 32, height: 48, numChannels: 4);
      for (final pixel in image) {
        pixel.setRgba(200, 30, 40, 255);
      }
      ...
      for (final p in other) {
        p.setRgba(20, 150, 250, 255);
      }
      expect(VibeStyleFeatures.extract(other).first, isNot(feature.first));
      expect(
        VibeStyleFeatures.isValid(
          VibeStyleFeatures.extract(img.Image(width: 1, height: 1)),
        ),
        isTrue,
      );
    },
  ...
  test('multiple seeds of one prompt do not inflate independent evidence', () {
    ...
    final extra = VibeStyleCorpus.parse([
      row(5, 'a', prompt: 'second'),
      row(6, 'b', prompt: 'second'),
    ]);
    samples.addAll(extra);
    for (final s in extra) {
      features[s.cacheKey] = feature;
    }
  ...
  test(
    'co-used codes in one output and corrupt cached features do not match',
    () {
      final samples = VibeStyleCorpus.parse([row(1, 'a', other: 'b')]);
      final feature = VibeStyleFeatures.extract(
        img.Image(width: 16, height: 16),
      );
      expect(
        VibeStyleMatcher.rank(samples, {samples.first.cacheKey: feature}),
        isEmpty,
      );
      expect(
        VibeStyleMatcher.rank(samples, {
          samples.first.cacheKey: [
            [double.nan],
          ],
        }),
        isEmpty,
  ...
  test(
    'feature worker reads sources without changing bytes, metadata or timestamps',
    () async {
      final temporary = await Directory.systemTemp.createTemp(
        'vibe_style_readonly_',
      );
      ...
      final sample = VibeStyleSample(
        imageId: 1,
        path: file.path,
        size: before.size,
        modifiedAt: before.modified.millisecondsSinceEpoch,
        hash: 'a',
        recipe: '',
        promptKey: '',
        seed: 1,
      ...
      expect(
        img.decodePng(await file.readAsBytes())!.textData!['Comment'],
  ...
  test('timeouts and cancellation release worker and permit retry', () async {
    final worker = VibeStyleWorker();
    final timeout = worker.run<List<VibeStyleSample>>(
      'parse',
      <Map<String, Object?>>[],
      timeout: Duration.zero,
    );
    await expectLater(timeout, throwsA(isA<TimeoutException>()));
    final cancelled = worker.run<List<VibeStyleSample>>(
      'parse',
      <Map<String, Object?>>[],
    );
    worker.cancel();
    ...
    expect(
      await worker.run<List<VibeStyleSample>>(
        'parse',
        <Map<String, Object?>>[],
      ),
      isEmpty,
