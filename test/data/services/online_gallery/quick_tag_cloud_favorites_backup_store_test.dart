...
void main() {
  ...
  test(
    'uses only trusted resolver details and preserves unresolved metadata',
    () async {
      ...
      final native = repository.getByStableKey(
        _detail('known').item.stableKey,
      )!;
  ...
  test(
    'replace changes only QuickTagCloud and preserves other sources',
    () async {
      ...
      final foreignBefore = repository
          .getByStableKey(foreign.item.stableKey)!
          .toMap();
      ...
      expect(
        repository.getByStableKey(foreign.item.stableKey)!.toMap(),
        foreignBefore,
      );
  ...
  test(
    'rejects native changes after preview without changing favorites',
    () async {
      final preview = await _prepare(store);
  ...
  test(
    'rejects metadata changes after preview before creating a journal',
    () async {
      final preview = await _prepare(store);
  ...
  test(
    ...
    () async {
      ...
      final atlas = (exported['favorites']['atlas'] as List)
          .cast<Map<String, dynamic>>();
      expect(
        atlas.map((item) => item['entryId']),
        containsAll(['known', 'missing', 'later']),
      );
      expect(
        atlas.singleWhere((item) => item['entryId'] == 'missing')['note'],
  ...
  test(
    'uncommitted token restores previous metadata despite equal native sets',
    () async {
  ...
  test(
    'exports native community identities only in favorites.community',
    () async {
      ...
      expect(
        await QuickTagCloudFavoritesBackupCodec.decode(
          QuickTagCloudFavoritesBackupCodec.encode(exported),
        ),
        exported,
      );
  ...
  test(
    'merge keeps native community details and atlas folder memberships',
    () async {
      final detail = _communityDetail('existing');
      ...
      expect(repository.getByStableKey(detail.item.stableKey)!.toMap(), before);
      expect(exported['favorites']['community'], [
        'existing',
        'new-unresolved',
      ]);
  ...
  test(
    'replace keeps incoming community identities with original native details',
    () async {
      final keep = _communityDetail(
        'keep',
        title: 'Previously saved community',
      );
      ...
      final keepBefore = repository
          .getByStableKey(keep.item.stableKey)!
          .toMap();
      final foreignBefore = repository
          .getByStableKey(foreign.item.stableKey)!
          .toMap();
      final preview = await store.prepare(
        ...
        cancelToken: CancelToken(),
        resolve: (codexId, entryId, token) async => null,
        resolveCommunity: (entryId, token) async => entryId == 'keep'
            ? _communityDetail('keep', title: 'Fresh detail')
            : null,
      );
      ...
      expect(
        repository.getByStableKey(keep.item.stableKey)!.toMap(),
        keepBefore,
      );
      expect(
        repository.getByStableKey(foreign.item.stableKey)!.toMap(),
  ...
  test(
    'imports trusted community details and retains unresolved community IDs',
    () async {
      final calls = <String>[];
      final preview = await store.prepare(
      ...
      final native = repository.getByStableKey(
        _communityDetail('trusted').item.stableKey,
      )!;
...
Matcher get _throwsStale => throwsA(
  isA<QuickTagCloudBackupException>().having(
    (error) => error.code,
    'code',
    'stale',
  ),
...
Future<QuickTagCloudBackupPreview> _prepare(
  QuickTagCloudFavoritesBackupStore store,
) => store.prepare(
...
GalleryDetail _detail(
  ...
  item: GalleryItem(
    ...
    sourceId: source,
    site: source.key,
    title: title ?? 'Trusted $entryId',
  ),
  media: const [],
  prompt: 'trusted $entryId prompt',
  rawSourceMetadata: const {'codexId': 'book'},
);

GalleryDetail _communityDetail(String entryId, {String? title}) =>
    GalleryDetail(
      item: GalleryItem(
        id: 2,
        workId: 'community:$entryId',
        sourceId: _source,
        site: _source.key,
        title: title ?? 'Trusted community $entryId',
      ),
      media: const [],
      prompt: 'trusted community $entryId prompt',
...
Map<String, dynamic> _emptyDocument({List<String> community = const []}) => {
  'format': QuickTagCloudFavoritesBackupCodec.format,
  'version': 2,
  ...
  'folders': [
    {
      ...
      'coverItemId': 'book:missing',
    },
  ],
  'memberships': [
    {
      'itemKey': 'book:missing',
      'folderId': 'folder_a',
