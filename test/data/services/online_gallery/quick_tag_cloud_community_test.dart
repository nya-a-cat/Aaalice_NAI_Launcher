...
void _serviceTests() {
  group('QuickTagCloudCommunityService', () {
    ...
    test(
      'loads only the fixed public GET endpoint without redirects',
      () async {
        adapter.body = {
    ...
    test('propagates an already cancelled request without dispatch', () async {
      ...
      await expectLater(
        service.load(cancelToken: token),
        throwsA(
    ...
    test(
      'accepts JSON returned as text and preserves injected Dio ownership',
      () async {
...
void _identityTests() {
  test(
    'validates schema, skips malformed/duplicate IDs and freezes entries',
    () {
      for (final value in [
        null,
        [],
        ...
        expect(
          () => QuickTagCloudCommunityParser.parse(value),
  ...
  test('keeps stable community identity and an encoded source link', () {
    ...
    final revised = _parse([
      _entry(id, {'title': 'Changed'}),
    ]).single;
  ...
  test('normalizes published category aliases and defaults unknown values', () {
    ...
    for (final entry in cases.entries) {
      expect(
        QuickTagCloudCommunityParser.normalizeCategory(entry.key),
        entry.value,
        reason: '${entry.key}',
      );
    }
    expect(QuickTagCloudCommunityParser.normalizeCategory(['人物', '服装']), '人物');
  });
}

void _imageUrlTests() {
  test('accepts only trusted HTTPS images and approved relative paths', () {
    const imageUrl = QuickTagCloudCommunityParser.imageUrl;
    expect(
      imageUrl('cover.webp'),
  ...
  test('rejects unsafe image schemes, hosts, credentials and traversal', () {
    for (final value in [
      ...
      expect(
        QuickTagCloudCommunityParser.imageUrl(value),
        isEmpty,
        reason: value,
      );
    }
...
void _mediaTests() {
  test('maps prompts, characters, images, parameters and chosen cover', () {
    ...
    expect(detail.characterPrompts.map((item) => item.label), [
      'Alice',
      'char2',
    ]);
  ...
  test(
    'drops untrusted images, clamps covers and handles text-only entries',
    () {
      final detail = _parse([
        _entry('images', {
          'images': [
            null,
            {'file': 'https://arbitrary.example/image.png'},
...
void _ratingTests() {
  test('uses conservative ratings and ignores negative-prompt markers', () {
    const rate = QuickTagCloudCommunityParser.ratingFor;
    expect(rate({}), 'q');
    ...
    expect(
      rate({
        'nsfw': false,
        'negative': 'nsfw, nude, gore, r18g',
        'characterPrompts': [
          {'prompt': 'portrait', 'negative': 'guro, sex'},
        ],
      }),
      'g',
    );
...
void _queryTests() {
  group('QuickTagCloudCommunityQuery', () {
    test(
      'intersects search terms, category, favorites and selected ratings',
      () {
        ...
        const query = QuickTagCloudCommunityQuery(
          search: ' OCEAN alice Sunrise BLUE rain SAILOR hat ',
          category: '场景',
          favoritesOnly: true,
        );
        expect(
          query.apply(entries, favoriteKeys: {entries.first.item.stableKey}),
          [entries.first],
        );
        expect(query.apply(entries), isEmpty);
        expect(
          const QuickTagCloudCommunityQuery(category: '构图').apply(entries),
          isEmpty,
        );
        expect(
          const QuickTagCloudCommunityQuery(search: 'missing').apply(entries),
          isEmpty,
        );
      },
    );
    ...
    test(
      'requires both global and source access gates for restricted content',
      () {
        final entries = _parse([
          _entry('safe'),
          {'id': 'unknown'},
        ...
        expect(
          ids(const QuickTagCloudCommunityQuery(ratings: {'g', 'q', 'e'})),
          ['community:safe'],
        );
        expect(
          ids(
        ...
        expect(
          ids(
            const QuickTagCloudCommunityQuery(ratings: {'e'}, allowR18g: true),
          ),
          isEmpty,
        );
        expect(
    ...
    test('sorts popularity with newest-first ties without mutating source', () {
      ...
      final sorted = const QuickTagCloudCommunityQuery(
        popularFirst: true,
      ).apply(entries);
