...
void main() {
  test(
    'accepts website V1 without inventing snapshots or source details',
    () async {
      ...
      expect(
        QuickTagCloudBackupPlan.atlas(result).single.containsKey('snap'),
        isFalse,
      );
    },
  );
  ...
  test('V2 JSON and NAITAG1 gzip round trip all library metadata', () async {
    ...
    expect(
      await QuickTagCloudFavoritesBackupCodec.decode(
        QuickTagCloudFavoritesBackupCodec.encode(original),
      ),
      original,
    );
  });
  ...
  test('rejects oversized decoded gzip and invalid UTF8', () async {
    final bytes = gzip.encode(
      List.filled(QuickTagCloudFavoritesBackupCodec.maximumBytes + 1, 32),
    );
    await expectLater(
      QuickTagCloudFavoritesBackupCodec.decode(
  ...
  test(
    'rejects invalid items, versions, duplicate folder IDs and orphan relations',
    () async {
      for (final change in <void Function(Map<String, dynamic>)>[
        ...
        change(value);
        await expectLater(
          QuickTagCloudFavoritesBackupCodec.decode(jsonEncode(value)),
          throwsA(isA<QuickTagCloudBackupException>()),
        );
      }
    },
  ...
  test('merges by identity and preserves conflicting notes for export', () {
    ...
    expect(
      plan.document['aaalicePreservedConflicts'],
      contains(
        equals({
          'scope': 'item:book:entry',
          'field': 'note',
          'value': 'website note',
        }),
      ),
    ...
    expect(plan.document['favorites']['community'], [
      'community-1',
      'community-2',
    ]);
    expect(incoming['memberships'][0]['folderId'], 'web');
    final second = QuickTagCloudBackupPlan.create(
      plan.document,
      incoming,
      replace: false,
    );
  ...
  test(
    'replace remains valid when merged folder count would exceed the limit',
    () {
      final old = _document();
      old['folders'] = [
        for (var i = 0; i < 100; i++)
      ...
      final incoming = _document();
      incoming['folders'] = [
        {'id': 'new', 'name': 'New folder', 'order': 0},
      ];
      expect(
        () => QuickTagCloudBackupPlan.create(old, incoming, replace: false),
        throwsA(isA<QuickTagCloudBackupException>()),
      );
      expect(
        QuickTagCloudBackupPlan.create(
          old,
          incoming,
          replace: true,
...
Map<String, dynamic> _document({int version = 2}) => {
  ...
  'favorites': {
    'atlas': <Map<String, dynamic>>[
      {'codexId': 'book', 'entryId': 'entry'},
    ],
