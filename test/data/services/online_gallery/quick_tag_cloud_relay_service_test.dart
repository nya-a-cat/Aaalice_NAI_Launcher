...
void main() {
  ...
  test('NAI block weights preserve whole comma groups', () {
    ...
    expect(service.compileBlock('cat', QuickTagCloudRelayFormat.nai, 1), 'cat');
  });
  ...
  test('plain preserves inline syntax and ignores outer block weight', () {
    expect(
      service.compileBlock(
        '{{cat}}, 1.25::sky::',
        QuickTagCloudRelayFormat.plain,
        2,
      ),
      '{{cat}}, 1.25::sky::',
    );
  ...
  test('empty numeric segments retain following delimiters and content', () {
    expect(
      QuickTagCloudRelayService.naiToSd('1::, cat, 2::dog::'),
      ', cat, (dog:2)',
    );
  ...
  test('top-level splitting protects weighted and bracket comma groups', () {
    ...
    expect(QuickTagCloudRelayService.splitTopLevel('1.2::cat, dog, tree'), [
      '1.2::cat',
      'dog',
      'tree',
    ]);
  ...
  test(
    'deduplication normalizes case and spaces independently per channel',
    () {
      final source = plan([
  ...
  test(
    'disabled and locked blocks are excluded and unknown ratings fail closed',
    () {
      final source = plan([
  ...
  test('characters retain independent positive and negative text', () {
    ...
    expect(
      output.allText,
      contains('Character 1: Alice\n1.2::cat::\nNegative:\n1.2::tired::'),
    );
    expect(
      output.allText,
      contains('Character 2: Bob\n1.2::cat::\nNegative:\n1.2::angry::'),
    );
  });
  ...
  test('stored restricted blocks obey current source access switches', () {
    ...
    expect(output.lockedCount, 2);
    expect(
      QuickTagCloudRelayService.effectiveRatings(
        {'g', 'q', 'e'},
        allowNsfw: true,
        allowR18g: false,
      ),
  ...
  test(
    'JSON roundtrip retains plans, source metadata and character negatives',
    () {
    ...
    for (final override in [
      ...
      expect(
        () => QuickTagCloudRelayDocument.fromJson({
          ...QuickTagCloudRelayDocument.empty().toJson(),
          ...override,
        }),
        throwsA(isA<Object>()),
      );
