...
void main() {
  test('search grammar preserves phrases, exclusions, aliases and invalid input', () {
    ...
    expect(plan.filters.map((filter) => filter.field), ['default', 'title', 'has', 'fav']);
    ...
    expect(QuickTagCloudSearchParser.parse('unknown:foo').terms, ['unknown:foo']);
    for (final query in [
      'title:', 'title:"unfinished', 'has:maybe', '-fav:true',
      'fav:true fav:false', 'title:a -title:a', 'type:unknown', '""',
      List.generate(11, (index) => 'term$index').join(' '),
    ]) {
      expect(QuickTagCloudSearchParser.parse(query).hasErrors, isTrue, reason: query);
    }
    expect(QuickTagCloudSearchParser.parse('', filterValues: ['unknown:foo']).hasErrors, isTrue);
    ...
    const filter = QuickTagCloudSearchFilter(
      field: 'title', value: 'Moon "Knight" \\ example', excluded: true,
    );
  ...
  test('AND and explicit fields match complete source metadata and exact directories', () {
    final document = QuickTagCloudSearchDocument(_records().first);
    bool matches(String query) => document.matches(QuickTagCloudSearchParser.parse(query));
    expect(matches('hero curator "red hair"'), isTrue);
    expect(matches('hero missing'), isFalse);
    expect(matches('hero -"red hair"'), isFalse);
    expect(matches('title:hero tag:"red hair" negative:lowres note:curator '
        'author:photographer codex:book type:画风 path:People has:image raw:original'), isTrue);
  ...
  test('engine reads fresh unified favorite keys and keeps category and rating boundaries', () async {
    ...
    Future<List<QuickTagCloudGalleryRecord>> search(String query) => engine.matchingRecords(
    ...
    expect((await search('fav:true')).map((item) => item.entry.id), ['concept']);
    expect((await search('fav:false')).map((item) => item.entry.id), ['hero']);
    expect(await engine.matchingRecords(
      const QuickTagCloudGalleryQuery(codexId: 'book', categoryPath: ['People']),
      searchText: 'fav:true',
      selectedRatings: {'g'},
    ), isEmpty);
  });

  test('invalid and cancelled queries stop before catalog work or favorite loading', () async {
    final user = QuickTagCloudUserService(_MemoryStorage());
    ...
    final engine = QuickTagCloudQueryEngine(
      repository: repository,
      userService: user,
      favoriteKeysLoader: () async { favoriteLoads++; return {}; },
    );
    await expectLater(engine.matchingRecords(
      const QuickTagCloudGalleryQuery(codexId: 'book'),
      searchText: 'has:maybe', selectedRatings: {'g'},
    ), throwsA(isA<QuickTagCloudSearchException>()));
    final cancelled = CancelToken()..cancel('test');
    await expectLater(engine.matchingRecords(
      const QuickTagCloudGalleryQuery(codexId: 'book'),
      searchText: 'fav:true', selectedRatings: {'g'}, cancelToken: cancelled,
...
List<QuickTagCloudGalleryRecord> _records() {
  ...
  final codex = QuickTagCloudParser.parseCodex(const {
    ...
    'entries': [
      {
        'id': 'hero', 'title': 'Moon Hero', 'path': ['People', 'Fantasy'],
        'tags': 'cinematic lighting', 'note': 'curator details',
        'images': [{'path': 'hero.webp', 'author': 'Photographer', 'rawTag': 'original'}],
        'characterPrompts': [{'prompt': 'red hair', 'negative': 'lowres'}],
      },
      {'id': 'concept', 'title': 'Text concept', 'path': ['Concepts'], 'tags': 'design'},
      {'id': 'adult', 'title': 'Adult', 'path': ['NSFW'], 'tags': 'design'},
    ],
  }, meta);
  final media = QuickTagCloudParser.parseMedia(const {});
  return codex.entries.map((entry) => QuickTagCloudGalleryRecord(meta, codex, entry, media)).toList();
}

class _Repository extends QuickTagCloudGalleryRepository {
  _Repository(QuickTagCloudUserService user, this.records)
    : super(catalogService: QuickTagCloudRemoteCatalogService(), userService: user);
