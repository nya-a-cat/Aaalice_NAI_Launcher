...
- **法典图鉴（NovelAI QuickTagCloud）**：按法典、分类、更新批次、图片状态和完整文本字段浏览固定公开法典，支持多图与纯文本词条、贡献者署名、正负及多角色 Prompt 复制/生成/排队、本地收藏和最近浏览。内容分级复用画廊已有的分级选单，提供全年龄、可疑（成人）与限制级（R18G / 重口）三级筛选。法典版本 JSON 经大小与 SHA-256 校验后缓存，图片仅使用普通运行时网络缓存；所有内容始终从上游读取，不随安装包分发或镜像。
  - 高级搜索支持引号短语、排除项、字段条件、图片与收藏筛选，提供可视化条件编辑器。
  - 提示词中转站在本机保存多个方案，可编辑、启停、排序与加权片段，切换 NAI / SD / 纯文本格式，并复制或填入生成页；多角色提示词保留独立角色槽。
  - 收藏支持网站 JSON / `NAITAG1` 迁移文本的预览、合并、覆盖与导出；覆盖仅影响法典来源。无法解析的条目、共创收藏及网站收藏夹信息保留用于重新导出。
  - 共创广场支持公开作品的搜索、分类、热度排序、详情与本地收藏，并沿用内容分级和黑名单。投稿入口打开原站，应用内不提交投稿或点赞。
