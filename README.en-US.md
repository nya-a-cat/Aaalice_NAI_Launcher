...
| 🌈 Reference & Style | Supports Vibe Transfer, Precise Reference, multi-image references, Vibe pack import/export, PNG metadata embedding/export, and local-gallery Vibe discovery with multiple examples for each exact encoding. Style families offer offline cross-encoding similarity suggestions with manual confirmation, splitting, and separation rules. Analysis reads original images without modifying them and requires no local deep-learning model or online image generation. |
...
- **Codex Gallery (NovelAI QuickTagCloud)**: Browses fixed public codexes by codex, category, update batch, image state, and full text fields. Supports multi-image and text-only entries, contributor attribution, positive/negative/multi-character Prompt copying, generation and queue handoff, local favorites, and recent history. Content ratings reuse the gallery's existing selector with General, Questionable (adult), and Explicit (R18G / extreme) filters. Versioned codex JSON is cached only after size and SHA-256 verification, while images use the normal runtime network cache; all content remains upstream-only and is neither bundled nor mirrored.
  - Advanced search supports quoted phrases, exclusions, field conditions, image and favorite filters, with a visual condition editor.
  - The prompt relay saves multiple plans locally, with editable, enabled/disabled, ordered and weighted blocks, NAI / SD / plain-text output, copying, and generation-page handoff. Multi-character prompts retain separate character slots.
  - Favorites support previewing, merging, replacing, and exporting website JSON / `NAITAG1` transfer text. Replacement affects only Codex Gallery favorites. Unresolved entries, community favorites, and website folder metadata are retained for re-export.
  - The community browser supports public-entry search, categories, popularity sorting, details, and local favorites, using the existing ratings and blacklist. The submission link opens the source site; the app does not submit entries or likes.
